...
*/
  //oop 用物件導向的方式定義POLYGON
    var area = function(arr,popup){ // 輸入一系列area資料 popup互動跳出文字
      this.arr = arr;
      this.popup = popup;
      ...
      this.polygon.on('mouseover',()=>{
        console.log('bb');
        this.polygon.setStyle({color:'#ff6347'})
      })
      this.polygon.on('mouseout',()=>{
        console.log('ll');
        this.polygon.setStyle({color:'#98fb98'})
      })
      // setting hover      
    ...
    $('#ubutton').click(()=>{
        if(p5j.arrarea.length>0){
          ...
          let exeurl = 'https://script.google.com/macros/s/AKfycbwoOcy-u-8vl1uQEAcC9Xp8ZQK51-qcwHufEGUfMjh-tvtjm4A/exec';          
          let editurl = 'https://docs.google.com/spreadsheets/d/1ErFqa73jkOEUnzgBaZn4eep-sWsL-bK4kL1TzcWHOXk/edit#gid=0';
